//Matthew Hill
//Assignment 15 - Revised Assignment 12
/*Objectives:
    1. Use dynamic allocation of memory.
*/
...
void process_file(vector<Product*> &product_list, int &num_products, string &file_name)
{
    ...
    for(int i=0;i<num_products;i++) //For all products
    {
        ...
        (*product_list[i]).number = atoi((contents_of_file[i][0]).c_str());
        (*product_list[i]).description = contents_of_file[i][1];
        (*product_list[i]).unit_price = atof((contents_of_file[i][2]).c_str());
        (*product_list[i]).quantity = atoi((contents_of_file[i][3]).c_str());
    }
}
...
void update_file(vector<Product*> &product_list, int &num_products, string &file_name)
{
    ...
    if(a_file.is_open())    //Tests if file opened properly
    {
        for(int i=0;i<num_products;i++) //For all products
        {
            ...
            a_file << (*product_list[i]).number << ",";
            a_file << (*product_list[i]).description << ",";
            a_file << (*product_list[i]).unit_price << ",";
            a_file << (*product_list[i]).quantity << "\n";
        }
